import requests
from configure import url_server


# test 1
def test_get_pokemons_by_types():
    pokemons = requests.get(url=f'{url_server}/getPokemonByType?type=normal').json()
    pokemons = pokemons.get('pokemons')
    assert "eevee" in pokemons
    result = requests.put(url=f'{url_server}/updateType?name=eevee')
...
def test_add_pokemon():
    ...
    add_pokemon = requests.post(url=f'{url_server}/addPokemon',
    ...
    pokemons = requests.get(url=f'{url_server}/getPokemonByType?type=bug').json().get('pokemons')
    assert "yanma" in pokemons

    # assert that added to types table that yanma have "flying" in its types
    pokemons = requests.get(url=f'{url_server}/getPokemonByType?type=flying').json().get('pokemons')
    assert "yanma" in pokemons
...
def test_update_pokemon_types():
    result = requests.put(url=f'{url_server}/updateType?name=venusaur')
    assert result.status_code == 200
    pokemons = requests.get(url=f'{url_server}/getPokemonByType?type=poison').json().get('pokemons')
    assert "venusaur" in pokemons
    pokemons = requests.get(url=f'{url_server}/getPokemonByType?type=grass').json().get('pokemons')
    assert "venusaur" in pokemons
...
def test_get_pokemons_by_owner():
    ...
    pokemons = requests.get(url=f'{url_server}/getPokemonsByTrainer?trainer=Drasna').json().get('pokimons')
...
def test_get_owners_by_pokemon():
    ...
    trainers = requests.get(url=f'{url_server}/getTrainersByPokemon?name=charmander').json().get('trainers')
...
def test_evolve():
    ...
    result = requests.put(url=f'{url_server}/evolve', json={"pokemon_name": "pinsir", "trainer_name": "Whitney"})
    assert result.status_code != 200

    # try evolve pokemon that not owned by this trainer
    result = requests.put(url=f'{url_server}/evolve', json={"pokemon_name": "spearow", "trainer_name": "Archie"})
    assert result.status_code != 200

    # evolve pokemon
    result = requests.put(url=f'{url_server}/evolve', json={"pokemon_name": "oddish", "trainer_name": "Whitney"})
    ...
    pokemons = requests.get(url=f'{url_server}/getPokemonsByTrainer?trainer=Whitney').json().get('pokimons')
    ...
    # evolve pikachu
    result = requests.put(url=f'{url_server}/evolve', json={"pokemon_name": "pikachu", "trainer_name": "Whitney"})
